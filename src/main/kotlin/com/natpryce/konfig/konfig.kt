...
/**
 * Validate a configuration against it's model [PropertyKeys]
 *
 * 1. all non optional keys in the model need to be set.
 * 2. all keys thad have a list of required on keys will be checked for presents or value equality,
 *    if true than the key will be required.
 *
 * @param model PropertyKeys the model to validate
 */
@Throws(Misconfiguration::class)
fun Configuration.validate(model: PropertyKeys): Configuration {

    val missingKeys = model.filter {

        // 1. check for keys with dependencies
        if (!it.requiredOn.isNullOrEmpty()) {
            it.requiredOn.forEach { required ->
                if (required.second == null && !contains(required.first)) {
                    // the config dose not contain the required key
                    return@filter true
                } else if (required.second != null && contains(required.first)) {
                    if (get(required.first) == (required.second)) {
                        // the required key's value is set and matched
                        return@filter !contains(it) // return `true` as missing if this key can not be found in the configs
                    }
                }
                return@filter false
            }
        ...
        if (!it.requiredOn.isNullOrEmpty()) {
            "${it.name} is required if ${it.requiredOn.joinToString { dep ->
                if (dep.second != null) {
                    "${dep.first.name} == ${dep.second}"
                } else {
                    "${dep.first.name} is present"
                }
            }}"
        } else {
            it.name
    ...
    if (missingKeys.isNotEmpty()) {
        throw Misconfiguration("Missing Keys: ${missingKeys.joinToString()}")
    }
    return this
}
